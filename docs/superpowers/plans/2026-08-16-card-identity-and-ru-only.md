...
- Тесты гоняются из корня репозитория: `pytest`. **Набор не зелёный и до этой ветки:** на `master` `pytest -q` даёт `7 failed, 332 passed, 1 xfailed, 188 errors`. Ошибки — `ScopeMismatch` в async-фикстурах и отсутствие Redis на `localhost:6379`; падения — `test_bot` (2), `test_config` (3), `test_handlers_comprehensive` (2). Всё это предшествует ветке. Критерий «зелено» для задачи: прибавились проходящие тесты и **не появилось новых** падений. Чинить эту базовую линию — не наша работа.
