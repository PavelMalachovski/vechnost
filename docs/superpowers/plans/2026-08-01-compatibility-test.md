...
- **Lint:** CI runs `ruff check .`. The repo is already red — 527 errors on master, none of them ours. The bar is **zero new findings**, measured as the branch-wide total not exceeding master's: `python -m ruff check . | tail -1`. It is **not** "every file you touched is clean" — appending to a file that already has pre-existing findings does not make them yours, and cleaning them up inflates the diff a reviewer has to read. Fix what your own lines introduce; leave the rest. Traps that have cost fix rounds: never place an import anywhere but the top-of-file block (E402), prefer `str | None` over `Optional[str]` (UP007), `@cache` over `@lru_cache(maxsize=None)` (UP033), and `zip(...)` needs `strict=` (B905).
- **Async tests must use the repo's working pattern.** `tests/conftest.py` defines a session-scoped `event_loop` fixture that makes async test functions and async fixtures error on collection — that is the bulk of the suite's 185 baseline errors. The pattern that works here is a **synchronous** test that calls `asyncio.run()` internally, as `tests/test_rooms.py::test_expired_room_410` does. A test that errors is worse than no test: it looks like coverage and provides none.
